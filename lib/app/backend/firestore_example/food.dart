...
CollectionReference _configDocRef() {
...
class Food {
  ...
  static Food fromDataString(String dataString) {
  ...
  static Food fromMap(Map<dynamic, dynamic> queryParams) {
    Food object = Food();
...
class FoodFireStore {
  CollectionReference<Food> _getObjectReference() {
    return _configDocRef().withConverter<Food>(
        fromFirestore: (snapshot, _) => Food.fromMap(
              snapshot.data()!,
            ),
        toFirestore: (object, _) => (object as Food).toMap());
  }

  Future<Food?> get(String docId) async {
    DocumentSnapshot<Food> documentSnapshot =
        await _getObjectReference().doc(docId).get();
  ...
  Future<void> upsert(Food object) async {
    await _getObjectReference()
        .doc(object.docId)
        .set(object, SetOptions(merge: true));
  }

  Future<void> insertBulk(List<Food> objects) async {
    ...
    for (int i = 0; i < objects.length; i += 500) {
      ...
      for (int j = i; j < i + 500 && j < objects.length; j++) {
        Food object = objects[j];
        batch.set(_getObjectReference().doc(object.docId), object);
      }

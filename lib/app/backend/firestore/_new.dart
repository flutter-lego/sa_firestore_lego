...
class NewFireStore {
  CollectionReference<New> getObjectReference() {
    return configDocRef().withConverter<New>(
        fromFirestore: (snapshot, _) => New.fromMap(
              snapshot.data()!,
            ),
        toFirestore: (object, _) => (object as New).toMap());
  }
  ...
  Future<void> upsert(New object) async {
    await getObjectReference()
        .doc(object.docId)
        .set(object, SetOptions(merge: true));
  }

  Future<void> insertBulk(List<New> objects) async {
